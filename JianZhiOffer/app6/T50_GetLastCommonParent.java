...
/**  
 * 面试题50：找两个路径中的最后一个共同的结点
 *    
 *  
 * @author 郑元浩 
 * @date 2017年4月6日 上午10:35:48 
 */
public class T50_GetLastCommonParent {
...
//		public TreeNode() {}
	...
	public static void getNodePath(TreeNode root, TreeNode target, List<TreeNode> path) {
		...
		for (TreeNode node : children) {
			if (node == target) { // 当前结点为目标结点，将该结点加到路径中
				path.add(node);
				return ;
			} else {
				getNodePath(node, target, path); // 得到结点的路径
			}
		}
		// 现场还原
