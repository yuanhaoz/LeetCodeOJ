...
public class BinaryTree {
    ...
    public static int getDepth(TreeNode root) {
    	...
    	while (!queue.isEmpty()) {
			TreeNode cur = queue.remove(); // 从对头位置移除
			currentLevelNodes--; // 减少当前level node的数量
			if (cur.left != null) { // 如果有左孩子，加到对尾
				queue.add(cur.left);
    ...
    */ 
    ...
    public static void postorderTraversal(TreeNode root) {
    	...
    	Stack<TreeNode> stack = new Stack<TreeNode>(); // 第一个栈用于添加node和它的左右孩子
    	Stack<TreeNode> output = new Stack<TreeNode>(); // 第二个栈用于翻转第一个stack的输出
    	stack.push(root);
    	while (!stack.isEmpty()) { // 确保所有元素都被翻转转移到第二个栈
    		TreeNode cur = stack.pop(); // 把栈顶元素添加到第二个栈
    		output.push(cur);
    		if (cur.left != null) {
				stack.push(cur.left);
			}
    ...
    public static int getNodeNumKthLevelRec(TreeNode root, int k) {
    	...
    	int left = getNodeNumKthLevelRec(root.left, k - 1); // 求root左子树的k-1层节点数
    	int right = getNodeNumKthLevelRec(root.right, k - 1); // 求root右子树的k-1层节点数
    	return left + right;
    }
    ...
    public static boolean isSameRec(TreeNode r1, TreeNode r2) {
    	...
    	boolean left = isSameRec(r1.left, r2.left); // 比较对应左子树
    	boolean right = isSameRec(r1.right, r2.right); // 比较对应右子树
    	return left && right;
    }
    ...
    /**
     * 非递归解法：
     * 先求从根节点到两个节点的路径， 然后再比较对应路径的节点就行，最后一个相同的节点就是它们在二叉树中的最低公共祖先节点
     */
    ...
    private static boolean getNodePath(TreeNode root, TreeNode node, ArrayList<TreeNode> path){
    	...
    	if (!found) { // 如果实在没有找到证明这个结点不在路径中，说明刚才添加进去的不是路径上的节点，删除
			path.remove(root);
		}
    	return found;
    }
     ...
     * 情况B：路径不经过根节点，而是左子树或右子树的最大距离路径，取其大者。
     * 只需要计算这两种的路径距离，并取其大者，就是该二叉树的最大距离。
     */
    public static Result getMaxDistanceRec(TreeNode root) {
    	if (root == null) {
			Result empty = new Result(0, -1); // 目的是让调用方+1后，把当前的不存在的（NULL）子树当成最大深度为0
			return empty;
		}
    	// 计算出左右子树分别最大距离
    ...
    public static TreeNode rebuildBinaryTreeRec(List<Integer> preOrder, List<Integer> inOrder) {
    	...
    	if (preOrder.size() != 0 && inOrder.size() != 0) {
			...
			// 因为知道root节点了，所以根据root节点位置，把preorder，inorder分别划分为root左侧和右侧的两个子区间
			inOrderPos = inOrder.indexOf(preOrder.get(0)); // inorder序列分割点
			...
			// root的左子树就是preorder和inorder的左侧区间而形成的树，右子树同理
    ...
    /**
     * 14. 判断二叉树是不是完全二叉树（迭代）
     * 若二叉树的深度为h，除第h层外，其它各层（1到h-1）的结点数都到达最大个数，
    ...
    public static boolean isCompleteBinaryTree(TreeNode root) {
    	...
    	while (!queue.isEmpty()) {
			...
			if (mustHaveNoChild) { // 已经出现了有空子树的节点了，后面出现的必须为叶节点（左右子树都为空）
				...
				if (cur.left != null && cur.right != null) { // 如果左右子树都非空，则继续遍历
					queue.add(cur.left);
					queue.add(cur.right);
				} else if (cur.left != null && cur.right == null) { // 如果左子树非空但右子树为空，说明已经出现空结点，之后必须都是空子树
					mustHaveNoChild = true;
					queue.add(cur.left);
				} else if (cur.left == null && cur.right != null) { // 如果左子树为空但右子树非空，说明已经不是完全二叉树！
					result = false;
					break;
